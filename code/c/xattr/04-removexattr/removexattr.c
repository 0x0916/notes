...
int main(int argc, char **argv) {
	...
	printf("## Remove File attr...\n");
	printf("## Remove attr user.fred\n");
	ret = removexattr(argv[1], "user.fred");
	if (ret < 0) {
		perror("removexattr");
		exit(EXIT_FAILURE);
	}
...
/*
# gcc -o removexattr removexattr.c 
# ./removexattr /tmp/foo 
## File attr
user.fred: chocolate
user.frieda: bar
...
user.joe: apple
## Remove File attr...
## Remove attr user.fred
## File attr(after remove):
user.frieda: bar
user.empty: <no value>
user.jim: banana
user.joe: apple
